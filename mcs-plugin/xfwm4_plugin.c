...
cb_layout_value_changed (GtkWidget * widget, gpointer user_data)
{
    ...
    for (i = TITLE; i < END; i++)
    {
        if ((i != row) && (title_radio_buttons[i].active == col))
        {
            ...
            {
                if ((i == TITLE)
                    && (title_radio_buttons[row].active == STATE_HIDDEN))
                {
                    gboolean in_use;
                ...
                else if ((col < STATE_HIDDEN)
                         && title_radio_buttons[row].active == j)
                {
                    gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON
                                                  (title_radio_buttons[i].
                                                   radio_buttons[j]), TRUE);
    ...
    mcs_manager_set_string (mcs_plugin->manager, "Xfwm/ButtonLayout", CHANNEL,
                            current_layout);
...
create_layout_buttons (gchar * layout, gpointer user_data)
{
    ...
    {
        temp =
            g_strdup_printf ("<small><i>%s :</i></small> ",
                             _(title_buttons[i].label));
        label_row = gtk_label_new (temp);
        gtk_widget_show (label_row);
        gtk_table_attach (GTK_TABLE (table), label_row, 0, 1, i + 1, i + 2,
        ...
        for (j = 0; j < STATES - 1; j++)
        {
            ...
            gtk_table_attach (GTK_TABLE (table), radio_button, j + 1, j + 2,
                              i + 1, i + 2, (GtkAttachOptions) (GTK_FILL),
                              (GtkAttachOptions) (0), 0, 0);
            ...
            g_signal_connect (G_OBJECT (radio_button), "toggled",
                              G_CALLBACK (cb_layout_value_changed),
                              callback_data);
            g_signal_connect_after (G_OBJECT (radio_button), "destroy",
                                    G_CALLBACK (cb_layout_destroy_button),
                                    callback_data);
        }
        ...
        {
            radio_button =
                gtk_radio_button_new_with_mnemonic (NULL, _("Hidden"));
            gtk_widget_show (radio_button);
            gtk_table_attach (GTK_TABLE (table), radio_button, STATES,
                              STATES + 1, i + 1, i + 2,
                              (GtkAttachOptions) (GTK_FILL),
                              (GtkAttachOptions) (0), 0, 0);
            gtk_radio_button_set_group (GTK_RADIO_BUTTON (radio_button),
...
static gboolean
parserc (const gchar * filename, gboolean * set_layout, gboolean * set_align,
         gboolean * set_font)
{
    gchar buf[80];
    gchar *lvalue, *rvalue;
...
static gboolean
dialog_update_from_theme (Itf * itf, const gchar * theme_name,
                          GList * theme_list)
{
    ThemeInfo *info = NULL;
    ...
    if (info)
    {
        gtk_container_foreach (GTK_CONTAINER (itf->frame_layout),
                               sensitive_cb,
...
decoration_selection_changed (GtkTreeSelection * selection, gpointer data)
{
    ...
    if (new_theme != NULL)
    {
        if (current_theme && strcmp (current_theme, new_theme))
        {
            ...
            dialog_update_from_theme (itf, current_theme,
                                      decoration_theme_list);
            mcs_manager_set_string (mcs_plugin->manager, "Xfwm/ThemeName",
                                    CHANNEL, current_theme);
            mcs_manager_notify (mcs_plugin->manager, CHANNEL);
            write_options (mcs_plugin);
        }
    }
...
keybinding_selection_changed (GtkTreeSelection * selection, gpointer data)
{
    ...
    if (gtk_tree_selection_get_selected (selection, &model, &iter))
    {
        gtk_tree_model_get (model, &iter, THEME_NAME_COLUMN, &new_key_theme,
                            -1);
    }
    else
    {
    ...
    if (new_key_theme != NULL)
    {
        if (current_key_theme && strcmp (current_key_theme, new_key_theme))
        {
            ...
            mcs_manager_set_string (mcs_plugin->manager, "Xfwm/KeyThemeName",
                                    CHANNEL, current_key_theme);
            mcs_manager_notify (mcs_plugin->manager, CHANNEL);
            write_options (mcs_plugin);
        }
    }
...
static GList *
read_themes (GList * theme_list, GtkWidget * treeview, GtkWidget * swindow,
             ThemeType type, gchar * current_value)
{
    ...
    for (list = new_list; list; list = list->next)
    {
        ...
        if (((type == DECORATION_THEMES) && !(info->has_decoration))
            || ((type == KEYBINDING_THEMES) && !(info->has_keybinding)))
            continue;

        gtk_list_store_prepend (GTK_LIST_STORE (model), &iter);
        gtk_list_store_set (GTK_LIST_STORE (model), &iter, THEME_NAME_COLUMN,
                            info->name, -1);
...
cb_click_to_focus_changed (GtkWidget * dialog, gpointer user_data)
{
    ...
    click_to_focus =
        gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON
                                      (itf->click_focus_radio));

    mcs_manager_set_int (mcs_plugin->manager, "Xfwm/ClickToFocus", CHANNEL,
                         click_to_focus ? 1 : 0);
    mcs_manager_notify (mcs_plugin->manager, CHANNEL);
    write_options (mcs_plugin);
}
...
cb_focus_new_changed (GtkWidget * dialog, gpointer user_data)
{
    ...
    mcs_manager_set_int (mcs_plugin->manager, "Xfwm/FocusNewWindow", CHANNEL,
                         focus_new ? 1 : 0);
    mcs_manager_notify (mcs_plugin->manager, CHANNEL);
    write_options (mcs_plugin);
}
...
cb_raise_on_focus_changed (GtkWidget * dialog, gpointer user_data)
{
    ...
    focus_raise =
        gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON
                                      (itf->raise_on_focus_check));
    gtk_widget_set_sensitive (itf->raise_delay_scale, focus_raise);

    mcs_manager_set_int (mcs_plugin->manager, "Xfwm/FocusRaise", CHANNEL,
                         focus_raise ? 1 : 0);
    mcs_manager_notify (mcs_plugin->manager, CHANNEL);
    write_options (mcs_plugin);
}
...
cb_raise_delay_changed (GtkWidget * dialog, gpointer user_data)
{
    ...
    raise_delay =
        (int) gtk_range_get_value (GTK_RANGE (itf->raise_delay_scale));
    mcs_manager_set_int (mcs_plugin->manager, "Xfwm/RaiseDelay", CHANNEL,
                         raise_delay);
    mcs_manager_notify (mcs_plugin->manager, CHANNEL);
    write_options (mcs_plugin);
}
...
cb_raise_on_click_changed (GtkWidget * dialog, gpointer user_data)
{
    ...
    raise_on_click =
        gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON
                                      (itf->click_raise_check));

    mcs_manager_set_int (mcs_plugin->manager, "Xfwm/RaiseOnClick", CHANNEL,
                         raise_on_click ? 1 : 0);
    mcs_manager_notify (mcs_plugin->manager, CHANNEL);
    write_options (mcs_plugin);
}
...
cb_snap_to_border_changed (GtkWidget * dialog, gpointer user_data)
{
    ...
    snap_to_border =
        gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON
                                      (itf->snap_to_border_check));
    gtk_widget_set_sensitive (itf->snap_width_scale, snap_to_windows
                              || snap_to_border);

    mcs_manager_set_int (mcs_plugin->manager, "Xfwm/SnapToBorder", CHANNEL,
                         snap_to_border ? 1 : 0);
    mcs_manager_notify (mcs_plugin->manager, CHANNEL);
    write_options (mcs_plugin);
}
...
cb_snap_to_windows_changed (GtkWidget * dialog, gpointer user_data)
{
    ...
    snap_to_windows =
        gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON
                                      (itf->snap_to_windows_check));
    gtk_widget_set_sensitive (itf->snap_width_scale, snap_to_windows
                              || snap_to_border);

    mcs_manager_set_int (mcs_plugin->manager, "Xfwm/SnapToWindows", CHANNEL,
                         snap_to_windows ? 1 : 0);
    mcs_manager_notify (mcs_plugin->manager, CHANNEL);
    write_options (mcs_plugin);
}
...
cb_snap_width_changed (GtkWidget * dialog, gpointer user_data)
{
    ...
    snap_width =
        (int) gtk_range_get_value (GTK_RANGE (itf->snap_width_scale));
    mcs_manager_set_int (mcs_plugin->manager, "Xfwm/SnapWidth", CHANNEL,
                         snap_width);
    mcs_manager_notify (mcs_plugin->manager, CHANNEL);
    write_options (mcs_plugin);
}
...
cb_wrap_resistance_changed (GtkWidget * dialog, gpointer user_data)
{
    ...
    wrap_resistance =
        (int) gtk_range_get_value (GTK_RANGE (itf->wrap_resistance_scale));
    mcs_manager_set_int (mcs_plugin->manager, "Xfwm/WrapResistance", CHANNEL,
                         wrap_resistance);
    mcs_manager_notify (mcs_plugin->manager, CHANNEL);
    write_options (mcs_plugin);
}
...
cb_wrap_workspaces_changed (GtkWidget * dialog, gpointer user_data)
{
    ...
    wrap_workspaces =
        gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON
                                      (itf->wrap_workspaces_check));
    gtk_widget_set_sensitive (itf->wrap_resistance_scale, wrap_workspaces
                              || wrap_windows);

    mcs_manager_set_int (mcs_plugin->manager, "Xfwm/WrapWorkspaces", CHANNEL,
                         wrap_workspaces ? 1 : 0);
    mcs_manager_notify (mcs_plugin->manager, CHANNEL);
    write_options (mcs_plugin);
}
...
cb_wrap_windows_changed (GtkWidget * dialog, gpointer user_data)
{
    ...
    wrap_windows =
        gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON
                                      (itf->wrap_windows_check));
    gtk_widget_set_sensitive (itf->wrap_resistance_scale, wrap_workspaces
                              || wrap_windows);

    mcs_manager_set_int (mcs_plugin->manager, "Xfwm/WrapWindows", CHANNEL,
                         wrap_windows ? 1 : 0);
    mcs_manager_notify (mcs_plugin->manager, CHANNEL);
    write_options (mcs_plugin);
}
...
cb_box_move_changed (GtkWidget * dialog, gpointer user_data)
{
    ...
    mcs_manager_set_int (mcs_plugin->manager, "Xfwm/BoxMove", CHANNEL,
                         box_move ? 1 : 0);
    mcs_manager_notify (mcs_plugin->manager, CHANNEL);
    write_options (mcs_plugin);
}
...
cb_box_resize_changed (GtkWidget * dialog, gpointer user_data)
{
    ...
    box_resize =
        !gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON
                                       (itf->box_resize_check));

    mcs_manager_set_int (mcs_plugin->manager, "Xfwm/BoxResize", CHANNEL,
                         box_resize ? 1 : 0);
    mcs_manager_notify (mcs_plugin->manager, CHANNEL);
    write_options (mcs_plugin);
}
...
font_selection_ok (GtkWidget * w, gpointer user_data)
{
    ...
    if (new_font != NULL)
    {
        if (current_font && strcmp (current_font, new_font))
        {
            ...
            gtk_button_set_label (GTK_BUTTON (itf->font_button),
                                  current_font);
            mcs_manager_set_string (mcs_plugin->manager, "Xfwm/TitleFont",
                                    CHANNEL, current_font);
            mcs_manager_notify (mcs_plugin->manager, CHANNEL);
            write_options (mcs_plugin);
        }
    }
...
show_font_selection (GtkWidget * widget, gpointer user_data)
{
    ...
    {
        itf->font_selection =
            gtk_font_selection_dialog_new (_("Font Selection Dialog"));

        gtk_window_set_position (GTK_WINDOW (itf->font_selection),
                                 GTK_WIN_POS_MOUSE);
        gtk_font_selection_dialog_set_font_name (GTK_FONT_SELECTION_DIALOG
                                                 (itf->font_selection),
                                                 current_font);
        g_signal_connect (itf->font_selection, "destroy",
                          G_CALLBACK (gtk_widget_destroyed),
                          &itf->font_selection);

        g_signal_connect (GTK_FONT_SELECTION_DIALOG (itf->font_selection)->
                          ok_button, "clicked",
                          G_CALLBACK (font_selection_ok), user_data);
        g_signal_connect_swapped (GTK_FONT_SELECTION_DIALOG
                                  (itf->font_selection)->cancel_button,
...
create_dialog (McsPlugin * mcs_plugin)
{
    ...
    gtk_box_pack_start (GTK_BOX (dialog->dialog_vbox), notebook, TRUE, TRUE,
                        0);
    ...
    gtk_container_add (GTK_CONTAINER (dialog->scrolledwindow1),
                       dialog->treeview1);
    ...
    gtk_container_add (GTK_CONTAINER (dialog->scrolledwindow2),
                       dialog->treeview2);
    ...
    gtk_box_pack_start (GTK_BOX (hbox), dialog->click_focus_radio, TRUE,
                        FALSE, 0);
    ...
    gtk_box_pack_start (GTK_BOX (hbox), dialog->focus_follow_mouse_radio,
                        TRUE, FALSE, 0);
    gtk_radio_button_set_group (GTK_RADIO_BUTTON
                                (dialog->focus_follow_mouse_radio),
                                dialog->click_focus_radio_group);
    ...
    gtk_box_pack_start (GTK_BOX (vbox4), dialog->raise_on_focus_check, FALSE,
                        FALSE, 0);
    gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON
                                  (dialog->raise_on_focus_check),
                                  focus_raise);
    ...
    gtk_widget_show (label);
    gtk_table_attach (GTK_TABLE (table2), label, 0, 3, 0, 1,
                      (GtkAttachOptions) (GTK_FILL), (GtkAttachOptions) (0),
                      0, 0);
    gtk_label_set_justify (GTK_LABEL (label), GTK_JUSTIFY_LEFT);
    gtk_misc_set_alignment (GTK_MISC (label), 0, 0.5);
    ...
    gtk_range_set_update_policy (GTK_RANGE (dialog->raise_delay_scale),
    ...
    gtk_box_pack_start (GTK_BOX (vbox5), dialog->snap_to_border_check, FALSE,
                        FALSE, 0);
    gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON
                                  (dialog->snap_to_border_check),
                                  snap_to_border);
    ...
    gtk_box_pack_start (GTK_BOX (vbox5), dialog->snap_to_windows_check, FALSE,
                        FALSE, 0);
    gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON
                                  (dialog->snap_to_windows_check),
                                  snap_to_windows);
    ...
    gtk_widget_show (label);
    gtk_table_attach (GTK_TABLE (table3), label, 0, 3, 0, 1,
                      (GtkAttachOptions) (GTK_FILL), (GtkAttachOptions) (0),
                      0, 0);
    gtk_label_set_justify (GTK_LABEL (label), GTK_JUSTIFY_LEFT);
    gtk_misc_set_alignment (GTK_MISC (label), 0, 0.5);
    ...
    dialog->snap_width_scale =
        gtk_hscale_new (GTK_ADJUSTMENT
                        (gtk_adjustment_new (snap_width, 5, 50, 5, 10, 0)));
    ...
    gtk_range_set_update_policy (GTK_RANGE (dialog->snap_width_scale),
                                 GTK_UPDATE_DISCONTINUOUS);
    ...
    gtk_box_pack_start (GTK_BOX (vbox6), dialog->wrap_workspaces_check, FALSE,
                        FALSE, 0);
    gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON
                                  (dialog->wrap_workspaces_check),
                                  wrap_workspaces);
    ...
    dialog->wrap_windows_check =
        gtk_check_button_new_with_mnemonic (_("Wrap workspaces when dragging a window off the screen"));
    gtk_widget_show (dialog->wrap_windows_check);
    gtk_box_pack_start (GTK_BOX (vbox6), dialog->wrap_windows_check, FALSE,
                        FALSE, 0);
    gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON
                                  (dialog->wrap_windows_check), wrap_windows);
    ...
    gtk_widget_show (label);
    gtk_table_attach (GTK_TABLE (table4), label, 0, 3, 0, 1,
                      (GtkAttachOptions) (GTK_FILL), (GtkAttachOptions) (0),
                      0, 0);
    gtk_label_set_justify (GTK_LABEL (label), GTK_JUSTIFY_LEFT);
    gtk_misc_set_alignment (GTK_MISC (label), 0, 0.5);
    ...
    gtk_box_pack_start (GTK_BOX (vbox7), dialog->box_resize_check, FALSE,
                        FALSE, 0);
    gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON
                                  (dialog->box_resize_check), !box_resize);
    ...
    gtk_box_pack_start (GTK_BOX (vbox7), dialog->box_move_check, FALSE, FALSE,
                        0);
    ...
    dialog->dialog_action_area1 =
        GTK_DIALOG (dialog->xfwm4_dialog)->action_area;
    gtk_widget_show (dialog->dialog_action_area1);
    gtk_button_box_set_layout (GTK_BUTTON_BOX (dialog->dialog_action_area1),
                               GTK_BUTTONBOX_END);
...
setup_dialog (Itf * itf)
{
    ...
    g_signal_connect (G_OBJECT (selection), "changed",
                      (GCallback) decoration_selection_changed, itf);

    selection = gtk_tree_view_get_selection (GTK_TREE_VIEW (itf->treeview2));
    gtk_tree_selection_set_mode (selection, GTK_SELECTION_BROWSE);
    g_signal_connect (G_OBJECT (selection), "changed",
                      (GCallback) keybinding_selection_changed, itf);

    decoration_theme_list =
        read_themes (decoration_theme_list, itf->treeview1,
                     itf->scrolledwindow1, DECORATION_THEMES, current_theme);
    keybinding_theme_list =
        read_themes (keybinding_theme_list, itf->treeview2,
                     itf->scrolledwindow2, KEYBINDING_THEMES,
                     current_key_theme);
    dialog_update_from_theme (itf, current_theme, decoration_theme_list);

    g_signal_connect (G_OBJECT (itf->xfwm4_dialog), "response",
                      G_CALLBACK (cb_dialog_response), itf->mcs_plugin);
    g_signal_connect (G_OBJECT (itf->font_button), "clicked",
                      G_CALLBACK (show_font_selection), itf);
    g_signal_connect (G_OBJECT (itf->click_focus_radio), "toggled",
                      G_CALLBACK (cb_click_to_focus_changed), itf);
    g_signal_connect (G_OBJECT (itf->focus_new_check), "toggled",
                      G_CALLBACK (cb_focus_new_changed), itf);
    ...
    g_signal_connect (G_OBJECT (itf->box_move_check), "toggled",
                      (GCallback) cb_box_move_changed, itf);
...
create_channel (McsPlugin * mcs_plugin)
{
    ...
    setting =
        mcs_manager_setting_lookup (mcs_plugin->manager, "Xfwm/KeyThemeName",
                                    CHANNEL);
    if (setting)
    {
    ...
    setting =
        mcs_manager_setting_lookup (mcs_plugin->manager, "Xfwm/ThemeName",
                                    CHANNEL);
    if (setting)
    {
        if (current_theme)
    ...
    setting =
        mcs_manager_setting_lookup (mcs_plugin->manager, "Xfwm/TitleFont",
                                    CHANNEL);
    if (setting)
    {
        if (current_font)
    ...
    setting =
        mcs_manager_setting_lookup (mcs_plugin->manager, "Xfwm/TitleAlign",
                                    CHANNEL);
    if (setting)
    {
        if (title_align)
    ...
    setting =
        mcs_manager_setting_lookup (mcs_plugin->manager, "Xfwm/ButtonLayout",
                                    CHANNEL);
    if (setting)
    {
        if (current_layout)
    ...
    else
    {
        ...
        mcs_manager_set_string (mcs_plugin->manager, "Xfwm/ButtonLayout",
                                CHANNEL, current_layout);
    }

    setting =
        mcs_manager_setting_lookup (mcs_plugin->manager, "Xfwm/ClickToFocus",
                                    CHANNEL);
    if (setting)
    {
    ...
    {
        click_to_focus = TRUE;
        mcs_manager_set_int (mcs_plugin->manager, "Xfwm/ClickToFocus",
                             CHANNEL, click_to_focus ? 1 : 0);
    }

    setting =
        mcs_manager_setting_lookup (mcs_plugin->manager,
                                    "Xfwm/FocusNewWindow", CHANNEL);
    if (setting)
    {
    ...
    {
        focus_new = TRUE;
        mcs_manager_set_int (mcs_plugin->manager, "Xfwm/FocusNewWindow",
                             CHANNEL, focus_new ? 1 : 0);
    }

    setting =
        mcs_manager_setting_lookup (mcs_plugin->manager, "Xfwm/FocusRaise",
                                    CHANNEL);
    if (setting)
    {
    ...
    setting =
        mcs_manager_setting_lookup (mcs_plugin->manager, "Xfwm/RaiseDelay",
                                    CHANNEL);
    if (setting)
    {
    ...
    setting =
        mcs_manager_setting_lookup (mcs_plugin->manager, "Xfwm/RaiseOnClick",
                                    CHANNEL);
    if (setting)
    {
    ...
    {
        raise_on_click = TRUE;
        mcs_manager_set_int (mcs_plugin->manager, "Xfwm/RaiseOnClick",
                             CHANNEL, raise_on_click ? 1 : 0);
    }

    setting =
        mcs_manager_setting_lookup (mcs_plugin->manager, "Xfwm/SnapToBorder",
                                    CHANNEL);
    if (setting)
    {
    ...
    {
        snap_to_border = TRUE;
        mcs_manager_set_int (mcs_plugin->manager, "Xfwm/SnapToBorder",
                             CHANNEL, snap_to_border ? 1 : 0);
    }

    setting =
        mcs_manager_setting_lookup (mcs_plugin->manager, "Xfwm/SnapToWindows",
                                    CHANNEL);
    if (setting)
    {
    ...
    else
    {
        ...
        mcs_manager_set_int (mcs_plugin->manager, "Xfwm/SnapToWindows",
                             CHANNEL, snap_to_windows ? 1 : 0);
    }

    setting =
        mcs_manager_setting_lookup (mcs_plugin->manager, "Xfwm/SnapWidth",
                                    CHANNEL);
    if (setting)
    {
    ...
    {
        wrap_resistance = 10;
        mcs_manager_set_int (mcs_plugin->manager, "Xfwm/WrapResistance",
                             CHANNEL, wrap_resistance);
    }

    setting =
        mcs_manager_setting_lookup (mcs_plugin->manager,
                                    "Xfwm/WrapWorkspaces", CHANNEL);
    if (setting)
    {
    ...
    setting =
        mcs_manager_setting_lookup (mcs_plugin->manager, "Xfwm/WrapWindows",
                                    CHANNEL);
    if (setting)
    {
    ...
    setting =
        mcs_manager_setting_lookup (mcs_plugin->manager, "Xfwm/BoxMove",
                                    CHANNEL);
    if (setting)
    {
    ...
    setting =
        mcs_manager_setting_lookup (mcs_plugin->manager, "Xfwm/BoxResize",
                                    CHANNEL);
    if (setting)
    {
