...
frameLeft (Client * c)
{
	...
	(c, CLIENT_FLAG_HAS_BORDER, CLIENT_FLAG_FULLSCREEN))
    {
...
frameHeight (Client * c)
{
    ...
    if (CLIENT_FLAG_TEST_AND_NOT
	(c, CLIENT_FLAG_HAS_BORDER | CLIENT_FLAG_SHADED,
	 CLIENT_FLAG_FULLSCREEN))
    {
	return frameTop (c) + frameBottom (c);
    }
    else if (CLIENT_FLAG_TEST_AND_NOT
	     (c, CLIENT_FLAG_HAS_BORDER, CLIENT_FLAG_FULLSCREEN))
    {
...
static inline void
fillRectangle (Display * dpy, Drawable d, Pixmap pm, int x,
...
static inline void
frameCreateTitlePixmap (Client * c, int state, int left,
			int right, MyPixmap * pm)
{
	...
	fillRectangle (dpy, pm->pixmap, params.title[TITLE_1][state].pixmap,
		       0, 0, w1, frameTop (c));
	fillRectangle (dpy, pm->mask, params.title[TITLE_1][state].mask, 0,
		       0, w1, frameTop (c));
	x = x + w1;
    }
	...
	fillRectangle (dpy, pm->pixmap, params.title[TITLE_3][state].pixmap,
		       x, 0, w3, frameTop (c));
	fillRectangle (dpy, pm->mask, params.title[TITLE_3][state].mask, x,
		       0, w3, frameTop (c));
	...
	fillRectangle (dpy, pm->pixmap, params.title[TITLE_5][state].pixmap,
		       x, 0, w5, frameTop (c));
	fillRectangle (dpy, pm->mask, params.title[TITLE_5][state].mask, x,
		       0, w5, frameTop (c));
    }
...
getButtonFromLetter (char chr, Client * c)
{
	...
	case 'T':
		...
		(c, CLIENT_FLAG_HAS_STICK | CLIENT_FLAG_HAS_MENU))
	    {
		b = STICK_BUTTON;
	    }
	    break;
...
static inline void
frameSetShape (Client * c, int state,
	       ClientPixmapCache * pm_cache, int button_x[BUTTON_COUNT])
{
    Window temp;
    int i;
    XRectangle rect;
    ...
    if (pm_cache)
    {
	XShapeCombineMask (dpy, MYWINDOW_XWINDOW (c->title), ShapeBounding,
	...
	if (!CLIENT_FLAG_TEST (c, CLIENT_FLAG_SHADED))
	{
	    XShapeCombineMask (dpy,
			       ...
			       ShapeBounding, 0, 0,
			       pm_cache->pm_sides[SIDE_LEFT][state].mask,
			       ShapeSet);
	    XShapeCombineMask (dpy,
			       MYWINDOW_XWINDOW (c->sides[SIDE_RIGHT]),
			       ShapeBounding, 0, 0,
			       pm_cache->pm_sides[SIDE_RIGHT][state].mask,
			       ShapeSet);
	}
	XShapeCombineMask (dpy, MYWINDOW_XWINDOW (c->sides[SIDE_BOTTOM]),
			   ShapeBounding, 0, 0,
			   pm_cache->pm_sides[SIDE_BOTTOM][state].mask,
			   ShapeSet);
	XShapeCombineMask (dpy,
			   MYWINDOW_XWINDOW (c->
					     corners[CORNER_BOTTOM_LEFT]),
			   ShapeBounding, 0, 0,
			   params.corners[CORNER_BOTTOM_LEFT][state].mask,
			   ShapeSet);
	XShapeCombineMask (dpy,
			   MYWINDOW_XWINDOW (c->
					     corners[CORNER_BOTTOM_RIGHT]),
			   ShapeBounding, 0, 0,
			   params.corners[CORNER_BOTTOM_RIGHT][state].mask,
			   ShapeSet);
	XShapeCombineMask (dpy,
			   MYWINDOW_XWINDOW (c->corners[CORNER_TOP_LEFT]),
			   ShapeBounding, 0, 0,
			   params.corners[CORNER_TOP_LEFT][state].mask,
			   ShapeSet);
	XShapeCombineMask (dpy,
			   MYWINDOW_XWINDOW (c->corners[CORNER_TOP_RIGHT]),
			   ShapeBounding, 0, 0,
			   params.corners[CORNER_TOP_RIGHT][state].mask,
			   ShapeSet);
	...
	XShapeCombineShape (dpy, temp, ShapeBounding,
			    ...
			    0, MYWINDOW_XWINDOW (c->title), ShapeBounding,
			    ShapeUnion);
	XShapeCombineShape (dpy, temp, ShapeBounding,
			    params.corners[CORNER_BOTTOM_LEFT][ACTIVE].
			    width, frameHeight (c) - frameBottom (c),
	...
	XShapeCombineShape (dpy, temp, ShapeBounding,
			    frameWidth (c) -
			    params.corners[CORNER_BOTTOM_RIGHT][ACTIVE].
			    width,
			    frameHeight (c) -
			    params.corners[CORNER_BOTTOM_RIGHT][ACTIVE].
			    height,
	...
	for (i = 0; i < BUTTON_COUNT; i++)
	{
		...
		XShapeCombineShape (dpy, temp, ShapeBounding,
				    button_x[i],
				    (frameTop (c) -
				     params.buttons[i][ACTIVE].height) /
				    2, MYWINDOW_XWINDOW (c->buttons[i]),
				    ShapeBounding, ShapeUnion);
	    }
	}
    }
...
frameDraw (Client * c, gboolean invalidate_cache, gboolean force_shape_update)
{
	...
	for (i = 0; i < strlen (params.button_layout); i++)
	{
		...
		myWindowShow (&c->buttons[button], x,
			      (frameTop (c) -
			       params.buttons[button][ACTIVE].height) /
			      2, params.buttons[button][ACTIVE].width,
	...
	fillRectangle (dpy, c->pm_cache.pm_sides[SIDE_LEFT][state].pixmap,
		       params.sides[SIDE_LEFT][state].pixmap, 0, 0,
		       frameLeft (c), left_height);
	fillRectangle (dpy, c->pm_cache.pm_sides[SIDE_LEFT][state].mask,
		       params.sides[SIDE_LEFT][state].mask, 0, 0,
		       frameLeft (c), left_height);
	...
	fillRectangle (dpy, c->pm_cache.pm_sides[SIDE_RIGHT][state].pixmap,
		       params.sides[SIDE_RIGHT][state].pixmap, 0, 0,
		       frameRight (c), right_height);
	fillRectangle (dpy, c->pm_cache.pm_sides[SIDE_RIGHT][state].mask,
		       params.sides[SIDE_RIGHT][state].mask, 0, 0,
		       frameRight (c), right_height);
	...
	fillRectangle (dpy, c->pm_cache.pm_sides[SIDE_BOTTOM][state].pixmap,
		       params.sides[SIDE_BOTTOM][state].pixmap, 0, 0,
		       bottom_width, frameBottom (c));
	fillRectangle (dpy, c->pm_cache.pm_sides[SIDE_BOTTOM][state].mask,
		       params.sides[SIDE_BOTTOM][state].mask, 0, 0,
		       bottom_width, frameBottom (c));
	...
	XSetWindowBackgroundPixmap (dpy,
				    MYWINDOW_XWINDOW (c->
						      sides[SIDE_BOTTOM]),
				    c->pm_cache.
				    pm_sides[SIDE_BOTTOM][state].pixmap);
	XSetWindowBackgroundPixmap (dpy,
				    MYWINDOW_XWINDOW (c->
						      corners
						      [CORNER_TOP_LEFT]),
				    params.corners[CORNER_TOP_LEFT][state].
				    pixmap);
	XSetWindowBackgroundPixmap (dpy,
				    MYWINDOW_XWINDOW (c->
						      corners
						      [CORNER_TOP_RIGHT]),
				    params.corners[CORNER_TOP_RIGHT][state].
				    pixmap);
	XSetWindowBackgroundPixmap (dpy,
				    MYWINDOW_XWINDOW (c->
						      corners
						      [CORNER_BOTTOM_LEFT]),
				    params.
				    corners[CORNER_BOTTOM_LEFT][state].
				    pixmap);
	XSetWindowBackgroundPixmap (dpy,
				    MYWINDOW_XWINDOW (c->
						      corners
						      [CORNER_BOTTOM_RIGHT]),
				    params.
				    corners[CORNER_BOTTOM_RIGHT][state].
				    pixmap);
	...
	else
	{
	    ...
	    myWindowShow (&c->sides[SIDE_RIGHT],
			  ...
			  frameRight (c), right_height, requires_clearing);
	}

	myWindowShow (&c->title,
		      params.corners[CORNER_TOP_LEFT][ACTIVE].width, 0,
		      top_width, frameTop (c), requires_clearing);
	myWindowShow (&c->sides[SIDE_BOTTOM],
		      params.corners[CORNER_BOTTOM_LEFT][ACTIVE].width,
		      frameHeight (c) - frameBottom (c), bottom_width,
		      frameBottom (c), requires_clearing);
	...
	myWindowShow (&c->corners[CORNER_TOP_RIGHT],
		      frameWidth (c) -
		      params.corners[CORNER_TOP_RIGHT][ACTIVE].width, 0,
		      params.corners[CORNER_TOP_RIGHT][ACTIVE].width,
		      params.corners[CORNER_TOP_RIGHT][ACTIVE].height,
		      requires_clearing);
