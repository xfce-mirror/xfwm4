...
clientGetWMNormalHints (Client *c, gboolean update)
{
    ...
    c->size->max_width  = MAX(c->size->max_height, 1);
    c->size->max_height = MAX(c->size->max_height, 1);

    /* apply constraints to window geometry */

    wc.width = MAX(MIN(wc.width, c->size->max_width), c->size->min_width);
    wc.height = MAX(MIN(wc.height, c->size->max_height), c->size->min_height);
