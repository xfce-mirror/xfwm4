...
grabKey (Display * dpy, MyKey * key, Window w)
{
	...
	else
	{
	    ...
	    XGrabKey (dpy, key->keycode, key->modifier, w, FALSE,
		      GrabModeAsync, GrabModeAsync);
	    XGrabKey (dpy, key->keycode, key->modifier | ScrollLockMask,
	    ...
	    XGrabKey (dpy, key->keycode,
		      ...
		      FALSE, GrabModeAsync, GrabModeAsync);
	    XGrabKey (dpy, key->keycode,
		      key->modifier | ScrollLockMask | CapsLockMask, w,
		      FALSE, GrabModeAsync, GrabModeAsync);
	    XGrabKey (dpy, key->keycode,
		      key->modifier | CapsLockMask | NumLockMask, w,
		      FALSE, GrabModeAsync, GrabModeAsync);
	    XGrabKey (dpy, key->keycode,
		      key->
		      modifier | ScrollLockMask | CapsLockMask |
		      NumLockMask, w, FALSE, GrabModeAsync, GrabModeAsync);
	}
    }
}
