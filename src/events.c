...
moveRequest (Client * c, XEvent * ev)
{
	...
	(c, CLIENT_FLAG_HAS_BORDER | CLIENT_FLAG_HAS_MOVE,
...
resizeRequest (Client * c, int corner, XEvent * ev)
{
	...
	(c, CLIENT_FLAG_HAS_RESIZE | CLIENT_FLAG_IS_RESIZABLE))
    {
...
handleButtonPress (XButtonEvent * ev)
{
	...
	state =
	    ev->
	    state & (ShiftMask | ControlMask | AltMask | MetaMask |
		     SuperMask | HyperMask);
	win = ev->subwindow;
...
handleConfigureRequest (XConfigureRequestEvent * ev)
{
    ...
    while (XCheckTypedWindowEvent
	   (dpy, ev->window, ConfigureRequest, &otherEvent))
    {
	...
	{
	    TRACE
		("client %s (0x%lx) is attempting to manipulate its frame!",
		 c->name, c->window);
...
handlePropertyNotify (XPropertyEvent * ev)
{
	...
	if (ev->atom == XA_WM_NORMAL_HINTS)
	{
	    ...
	    if (CLIENT_FLAG_TEST (c, CLIENT_FLAG_IS_RESIZABLE) !=
		previous_value)
	    {
	...
	{
	    TRACE
		("client \"%s\" (0x%lx) has received a XA_WM_NAME notify",
		 c->name, c->window);
	    if (c->name)
	    {
		free (c->name);
	...
	{
	    TRACE
		("client \"%s\" (0x%lx) has received a win_hints notify",
		 c->name, c->window);
	...
	{
	    TRACE
		("client \"%s\" (0x%lx) has received a net_wm_strut notify",
		 c->name, c->window);
...
handleClientMessage (XClientMessageEvent * ev)
{
	...
	{
	    TRACE
		("client \"%s\" (0x%lx) has received a win_workspace event",
		 c->name, c->window);
	...
	{
	    TRACE
		("client \"%s\" (0x%lx) has received a net_wm_state event",
		 c->name, c->window);
	...
	if (((ev->message_type == win_workspace) ||
	     (ev->message_type == net_current_desktop)) && (ev->format == 32))
	{
	    TRACE
...
show_popup_cb (GtkWidget * widget, GdkEventButton * ev, gpointer data)
{
	...
	g_signal_connect (GTK_OBJECT (getDefaultGtkWidget ()),
			  "button_press_event",
			  GTK_SIGNAL_FUNC (show_popup_cb), (gpointer) NULL);
