...
root_tile (ScreenInfo *screen_info)
{
    ...
    for (p = 0; p < 2; p++)
    {
        ...
        if ((result == Success) && (actual_type == misc_pixmap) && (actual_format == 32) && (nitems == 1))
        {
    ...
    if (!pixmap)
    {
        pixmap = XCreatePixmap (dpy, screen_info->xroot, 1, 1, DefaultDepth (dpy, screen_info->screen));
        fill = True;
    }
    pa.repeat = True;
...
static void
get_paint_bounds (CWindow *cw, int *x, int *y, int *w, int *h)
{
...
#endif
            format = XRenderFindVisualFormat (dpy, cw->attr.visual);
            pa.subwindow_mode = IncludeInferiors;
            cw->picture = XRenderCreatePicture (dpy, draw, format, CPSubwindowMode, &pa);
        }
...
add_damage (ScreenInfo *screen_info, XserverRegion damage)
{
    ...
    {
        XFixesUnionRegion (myScreenGetXDisplay (screen_info), screen_info->allDamage, screen_info->allDamage, damage);
...
static unsigned int
get_opacity_prop(CWindow *cw, unsigned int def)
{
    ScreenInfo *screen_info;
    Atom actual;
    int format;
...
{
    unsigned int opacity = get_opacity_prop (cw, (unsigned int) (OPAQUE * def));
...
static void
expose_root (ScreenInfo *screen_info, XRectangle *rects, int nrects)
{
...
unmap_win (CWindow *cw)
{
    ...
    if (cw->name_window_pixmap)
    {
        XFreePixmap (myScreenGetXDisplay (screen_info), cw->name_window_pixmap);
    ...
    if (cw->picture)
    {
        XRenderFreePicture (myScreenGetXDisplay (screen_info), cw->picture);
        cw->picture = None;
    }

    if (cw->borderSize)
    {
        XFixesDestroyRegion (myScreenGetXDisplay (screen_info), cw->borderSize);
        cw->borderSize = None;
    }

    if (cw->shadow)
    {
        XRenderFreePicture (myScreenGetXDisplay (screen_info), cw->shadow);
        cw->shadow = None;
    }
    if (cw->borderClip)
    {
        XFixesDestroyRegion (myScreenGetXDisplay (screen_info), cw->borderClip);
        cw->borderClip = None;
    }
...
static void
add_win (DisplayInfo *display_info, Window id, Client *c, XWindowAttributes *attr, Window prev)
{
    ScreenInfo *screen_info;
    CWindow *new;
    ...
    {
        memcpy (&new->attr, attr, sizeof (XWindowAttributes));
    }
    ...
    if (prev != None)
    {
        ...
        pcw = find_cwindow_in_screen (screen_info, prev);
        if (pcw)
        {
            GList *sibling;
            sibling = g_list_find (screen_info->cwindows, (gconstpointer) pcw);
            if (sibling)
            {
                screen_info->cwindows = g_list_insert_before (screen_info->cwindows, g_list_next (sibling), new);
                inserted = TRUE;
            }
        }
    }
...
void
restack_win (ScreenInfo *screen_info, CWindow *cw, Window new_above)
{
    Window old_above;
    GList *sibling;
    GList *next;
    ...
    if (old_above != new_above)
    {
        ...
        {
            CWindow *naw;
            naw = find_cwindow_in_screen (screen_info, new_above);
            sibling = g_list_find (screen_info->cwindows, (gconstpointer) naw);
        }

        screen_info->cwindows = g_list_remove (screen_info->cwindows, (gconstpointer) cw);
        screen_info->cwindows = g_list_insert_before (screen_info->cwindows, sibling, cw);
    }
}
...
destroy_win (ScreenInfo *screen_info, Window id, gboolean gone)
{
    ...
    if (cw)
    {
        ...
        if (cw->damage != None)
        {
            XDamageDestroy (myScreenGetXDisplay (screen_info), cw->damage);
            cw->damage = None;
        }
        screen_info->cwindows = g_list_remove (screen_info->cwindows, (gconstpointer) cw);
        g_free (cw);
...
compositorHandleCirculateNotify (DisplayInfo *display_info, XCirculateEvent *ev)
{
    ...
    restack_win (cw->screen_info, cw, new_above);
    cw->screen_info->clipChanged = TRUE;
    compositorRepairScreen (cw->screen_info);
}
...
compositorHandleReparentNotify (DisplayInfo *display_info, XReparentEvent *ev)
{
    ...
    TRACE ("entering compositorHandleReparentNotify");    
    g_return_if_fail (display_info);
    if (!(display_info->enable_compositor))
    {
        TRACE ("compositor disabled");    
    ...
    if (cw)
    {
        ...
        if (ev->parent = screen_info->xroot)
        {
            add_win (display_info, ev->window, NULL, NULL, None);
        }
        else
        {
            destroy_win (screen_info, ev->window, FALSE);
        }
        compositorRepairScreen (screen_info);
    }
}
...
compositorWindowMap (DisplayInfo *display_info, Window id)
{
    ...
    {
        add_win (display_info, id, NULL, NULL, None);
...
void
compositorAddWindow (DisplayInfo *display_info, Window id, Client *c, XWindowAttributes *attr)
{
#ifdef HAVE_COMPOSITOR
    CWindow *cw;
    ...
    {
        add_win (display_info, id, c, attr, None);
        compositorDoRepair (display_info);
    }
#endif /* HAVE_COMPOSITOR */
}
