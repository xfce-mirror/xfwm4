...
gridResizeEventFilter (XfwmEvent *event, gpointer data)
{
    ...
    else if (event->meta.type == XFWM_EVENT_BUTTON)
    {
                 ...
                 && !event->button.pressed && griddata->drag)
        {
            ...
            clientMaxSpace (c, &griddata->want_x, &griddata->want_y,
                    &griddata->want_width, &griddata->want_height, TRUE);

            mask = CWX | CWY | CWWidth | CWHeight;
            wc.x = griddata->want_x + griddata->x + frameExtentLeft (c);
            wc.y = griddata->want_y + griddata->y + frameExtentTop (c);
            wc.width = griddata->want_width \
                       - frameExtentLeft (c) - frameExtentRight (c);
            wc.height = griddata->want_height \
...
void clientGridResize (Client * c, XfwmEventButton *event)
{
    ScreenInfo *screen_info;
    DisplayInfo *display_info;
    GtkWindow *grid;
    ...
    if (event && event->pressed)
    {
        /* gobal mode: use current pointer coordinates */
        myScreenFindMonitorAtPoint (screen_info, event->x_root,
                                    event->y_root, &geometry);
    }
    else
    {
        /* window mode: use center of active window */
        gint x, y;

        x = c->x + c->width / 2;
        y = c->y + c->height / 2;
        myScreenFindMonitorAtPoint (screen_info, x, y, &geometry);
    }
    ...
    grid = gridCreate (screen_info, &griddata, &geometry);
