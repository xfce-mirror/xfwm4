...
loadMcsData (Settings rc[])
{
    ...
    if (client)
    {
	if (mcs_client_get_setting
	    (client, "Xfwm/ClickToFocus", CHANNEL1, &setting) == MCS_SUCCESS)
	{
	...
	if (mcs_client_get_setting
	    (client, "Xfwm/FocusNewWindow", CHANNEL1,
	     &setting) == MCS_SUCCESS)
	{
	...
	if (mcs_client_get_setting
	    (client, "Xfwm/FocusRaise", CHANNEL1, &setting) == MCS_SUCCESS)
	{
	...
	if (mcs_client_get_setting
	    (client, "Xfwm/RaiseDelay", CHANNEL1, &setting) == MCS_SUCCESS)
	{
	    setIntValueFromInt ("raise_delay", setting->data.v_int, rc);
	    mcs_setting_free (setting);
	}
	if (mcs_client_get_setting
	    (client, "Xfwm/RaiseOnClick", CHANNEL1, &setting) == MCS_SUCCESS)
	{
	...
	if (mcs_client_get_setting
	    (client, "Xfwm/SnapToBorder", CHANNEL1, &setting) == MCS_SUCCESS)
	{
	...
	if (mcs_client_get_setting
	    (client, "Xfwm/SnapToWindows", CHANNEL1, &setting) == MCS_SUCCESS)
	{
	    setBooleanValueFromInt ("snap_to_windows",
				    setting->data.v_int, rc);
	    mcs_setting_free (setting);
	}
	if (mcs_client_get_setting
	    (client, "Xfwm/SnapWidth", CHANNEL1, &setting) == MCS_SUCCESS)
	{
	    setIntValueFromInt ("snap_width", setting->data.v_int, rc);
	    mcs_setting_free (setting);
	}
	if (mcs_client_get_setting
	    (client, "Xfwm/WrapWorkspaces", CHANNEL1,
	     &setting) == MCS_SUCCESS)
	{
	    setBooleanValueFromInt ("wrap_workspaces",
				    setting->data.v_int, rc);
	    mcs_setting_free (setting);
	}
	if (mcs_client_get_setting
	    (client, "Xfwm/WrapResistance", CHANNEL1,
	     &setting) == MCS_SUCCESS)
	{
	...
	if (mcs_client_get_setting
	    (client, "Xfwm/BoxMove", CHANNEL1, &setting) == MCS_SUCCESS)
	{
	    setBooleanValueFromInt ("box_move", setting->data.v_int, rc);
	    mcs_setting_free (setting);
	}
	if (mcs_client_get_setting
	    (client, "Xfwm/BoxResize", CHANNEL1, &setting) == MCS_SUCCESS)
	{
	    setBooleanValueFromInt ("box_resize", setting->data.v_int, rc);
	    mcs_setting_free (setting);
	}
	if (mcs_client_get_setting
	    (client, "Xfwm/DblClickAction", CHANNEL1,
	     &setting) == MCS_SUCCESS)
	{
	...
	if (mcs_client_get_setting
	    (client, "Xfwm/ThemeName", CHANNEL1, &setting) == MCS_SUCCESS)
	{
	    setValue ("theme", setting->data.v_string, rc);
	    mcs_setting_free (setting);
	}
	if (mcs_client_get_setting
	    (client, "Xfwm/KeyThemeName", CHANNEL1, &setting) == MCS_SUCCESS)
	{
	    setValue ("keytheme", setting->data.v_string, rc);
	    mcs_setting_free (setting);
	}
	if (mcs_client_get_setting
	    (client, "Xfwm/ButtonLayout", CHANNEL1, &setting) == MCS_SUCCESS)
	{
	    setValue ("button_layout", setting->data.v_string, rc);
	    mcs_setting_free (setting);
	}
	if (mcs_client_get_setting
	    (client, "Xfwm/TitleAlign", CHANNEL1, &setting) == MCS_SUCCESS)
	{
	    setValue ("title_alignment", setting->data.v_string, rc);
	    mcs_setting_free (setting);
	}
	if (mcs_client_get_setting
	    (client, "Xfwm/TitleFont", CHANNEL1, &setting) == MCS_SUCCESS)
	{
	    setValue ("title_font", setting->data.v_string, rc);
	    mcs_setting_free (setting);
	}
	if (mcs_client_get_setting
	    (client, "Xfwm/LeftMargin", CHANNEL2, &setting) == MCS_SUCCESS)
	{
	    setIntValueFromInt ("margin_left", setting->data.v_int, rc);
	    mcs_setting_free (setting);
	}
	if (mcs_client_get_setting
	    (client, "Xfwm/RightMargin", CHANNEL2, &setting) == MCS_SUCCESS)
	{
	    setIntValueFromInt ("margin_right", setting->data.v_int, rc);
	    mcs_setting_free (setting);
	}
	if (mcs_client_get_setting
	    (client, "Xfwm/BottomMargin", CHANNEL2, &setting) == MCS_SUCCESS)
	{
	    setIntValueFromInt ("margin_bottom", setting->data.v_int, rc);
	    mcs_setting_free (setting);
	}
	if (mcs_client_get_setting
	    (client, "Xfwm/TopMargin", CHANNEL2, &setting) == MCS_SUCCESS)
	{
...
loadTheme (Settings rc[])
{
	...
	if (gdk_colormap_alloc_color
	    (gdk_colormap_get_system (), &params.title_colors[ACTIVE].col,
	     FALSE, FALSE))
	{
	    ...
	    gdk_gc_copy (params.title_colors[ACTIVE].gc,
			 get_style_gc (widget, "text", "selected"));
    ...
    {
	if (gdk_colormap_alloc_color
	    (gdk_colormap_get_system (), &params.title_colors[INACTIVE].col,
	     FALSE, FALSE))
	{
...
loadSettings (void)
{
    ...
    set_settings_margin (MARGIN_BOTTOM,
			 TOINT (getValue ("margin_bottom", rc)));
	...
	params.double_click_action = ACTION_SHADE;
	     ...
	     ("hide", getValue ("double_click_action", rc)))
    {
	params.double_click_action = ACTION_HIDE;
    }
    else if (!g_ascii_strcasecmp
	     ("maximize", getValue ("double_click_action", rc)))
    {
