...
static void sn_add_sequence(SnStartupSequence * sequence)
{
    ...
    {
        startup_sequence_timeout = g_timeout_add(1000, sn_startup_sequence_timeout, NULL);
    }
    sn_update_feedback();
}
...
static void sn_collect_timed_out_foreach(void *element, void *data)
{
...
static void sn_screen_event(SnMonitorEvent * event, void *user_data)
{
    const char *wmclass;
...
void sn_client_startup_properties(Client * c)
{
    ...
    if(startup_id == NULL)
    {
        ...
        while(tmp != NULL)
        {
            ...
            if((wmclass != NULL) && ((c->class.res_class && !strcmp(wmclass, c->class.res_class)) || (c->class.res_name && !strcmp(wmclass, c->class.res_name))))
            {
                sequence = tmp->data;
